...
from docker import ComposeEnv, DockerInDockerComposeAdapter, LocalComposeAdapter
...
def test_docker_compose_env_ok(in_docker_container: MagicMock) -> None:
    ...
    docker_compose_wrapper = ComposeEnv(OPENVPN_COMPOSE_TEMPLATE, {})
...
def test_docker_compose_renderer_local(in_docker_container: MagicMock, primary_host_ip: MagicMock) -> None:
    ...
    renderer = LocalComposeAdapter(OPENVPN_COMPOSE_TEMPLATE)
...
def test_docker_compose_renderer_dind(in_docker_container: MagicMock) -> None:
    ...
    renderer = DockerInDockerComposeAdapter(OPENVPN_COMPOSE_TEMPLATE)
